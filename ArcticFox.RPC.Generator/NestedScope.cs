...
namespace ArcticFox.RPC.Generator
{
    ...
    public class NestedScope
    {
        ...
        {
            // {public/private...} {abstract }{ref} partial {class/struct} {name}

            var modifier = namedTypeSymbol.DeclaredAccessibility.ToString().ToLowerInvariant();
            
            var @abstract = namedTypeSymbol.IsAbstract ? "abstract " : string.Empty;
            var @ref = (namedTypeSymbol.IsRefLikeType ? "ref " : string.Empty);
            var str = $"{modifier} {@abstract}{@ref}partial {TypeKindToStr(namedTypeSymbol.TypeKind)} {namedTypeSymbol.Name}";
            return str;
        }
